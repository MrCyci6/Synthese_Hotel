    ...
    class User {
        ...
        static function loginUser(string $email, string $password) {
            $statement = Database::preparedQuery(
                "SELECT id_user FROM users WHERE LOWER(email)=LOWER(?) AND hash=crypt(?, hash);",
                [$email, $password]
            );
        ...
        static function addUser(string $nom, string $prenom, string $adresse, string $email, string $password = "") {
            $password = $password == "" ? uniqid() : $password;
            
            Database::preparedQuery(
                "INSERT INTO users(nom, prenom, addresse, email, hash, banned) VALUES (?, ?, ?, ?, crypt(?, gen_salt('bf')), 0);",
